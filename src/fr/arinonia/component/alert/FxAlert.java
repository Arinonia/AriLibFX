...
public class FxAlert {
	...
	public FxAlert(AlertType type, String title, String header, String content, Exception exception,String lbl, String cssFile, Class<?> cls) {
		alert = new Alert(type);
		alert.setTitle(title);
		...
		GridPane.setVgrow(textArea, Priority.ALWAYS);
		GridPane.setHgrow(textArea, Priority.ALWAYS);
		dialogPane.getStylesheets().add(cls.getResource(cssFile).toExternalForm());
		dialogPane.getStyleClass().add("dialogs");
		GridPane expContent = new GridPane();
		expContent.setMaxWidth(Double.MAX_VALUE);
		expContent.add(label, 0, 0);
